...
// check if buffer has space - return -1 if full
...
void main (void) 
{
    ...
    while (1)
    {
		...
		BUZZER = 0;// turns buzzer on at the end of period 
		...
		sprintf(buff1, "Instant BPM: %.2f", bpm);
		
		printf( "\rT=%f s    ", period);
	
		LCDprint(buff1 ,1,8);
		
		int ret = decimator(bpm);
